...
def get_product_or_404(db: Session, product_id: int, company_id: int):
...
def list_company_products(
    ...
    return list_products(
        db=db, company_id=company_id, is_active=is_active, category=category
    )


@router.get("/unit-margin-ranking", response_model=list[ProductRead])
def list_company_unit_margin_ranking(
    is_active: bool = True,
    category: str | None = None,
